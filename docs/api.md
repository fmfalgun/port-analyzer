...
| `poc_count` | integer | Number of public PoC repositories on GitHub (nomi-sec); `0` = no PoC found or not yet checked |
| `poc_urls` | object[] \| null | Array of `{url, stars, name}` objects for each PoC repo; `null` if not yet checked |
...
| `affected` | string \| null | Affected products/components (may be JSON-encoded string) |
...
- A PoC column in the CVE table showing `[PoC:N]` badges with the count of public exploit repos
- A VARIoT section listing IoT-specific vulnerabilities when available
- A `## References` section at the end listing IANA, NVD port search, CISA KEV (when `kev_count > 0`), EPSS, MITRE ATT&CK, PoC-in-GitHub (when `poc_count > 0`), VARIoT (when `variot_vulns` is non-empty), and per-CVE and per-technique bullet links
